import { Client, Guild, TextChannel, MessageEmbed, EmbedFieldData } from 'discord.js';
	...
	switch (args[0]) {
		...
		case "testEvent":
			events.push(new Event({
				...
				endTime: DateTime.utc().plus({minute: 1}),
				module: "4TPV106U Test event"
			}));
			break;
...
export function broadcast(role: string, title:string, msg: MessageEmbed) {
	...
	client.guilds.cache.forEach(g => {
		...
		const c = g.channels.cache.find(c => c.name === process.env.CHANNEL_NAME) as TextChannel;
		if(!c)
			return;
